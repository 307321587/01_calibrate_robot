 ...
 */
template <typename T>
py::buffer_info get_buffer_info(T &) = delete;
template <typename T, template <typename> class Array, typename std::enable_if<std::is_same<vpArray2D<T>, Array<T>>::value, bool>::type = true>
...
void add_cpp_print_helper(PybindClass &pyCls, std::ostream &(T::*fn)(std::ostream &, const std::string &, bool) const)
{
    pyCls.def(
        "strCppCode",
        [fn](const T &self, const std::string &name = "A", bool byte_per_byte = false) -> std::string {
            std::stringstream ss;
...
void define_get_item_2d_array(PyClass &pyClass)
{
    pyClass.def("__getitem__", [](const Class &self, std::pair<int, int> pair) -> Item {
        ...
        {
            std::stringstream ss;
            ss << "Invalid indexing into a 2D array: got indices " << shape_to_string({i, j}) << " but array has dimensions " << shape_to_string({rows, cols});
    ...
    pyClass.def(
        "__getitem__",
        [](const Class &self, int i) -> np_array_cf<Item> {
            const int rows = (int)self.getRows();
            if (i >= rows || i < -rows)
            {
                std::stringstream ss;
                ss << "Invalid indexing into a 2D array: got row index " << shape_to_string({i}) << " but array has " << rows << " rows";
                throw std::runtime_error(ss.str());
            }
            if (i < 0)
            {
        ...
        py::keep_alive<0, 1>());
    pyClass.def(
        "__getitem__", [](const Class &self, py::slice slice) -> py::array_t<Item> { return (py::cast(self).template cast<np_array_cf<Item>>())[slice].template cast<np_array_cf<Item>>(); },
        py::keep_alive<0, 1>());
    pyClass.def(
        "__getitem__", [](const Class &self, py::tuple tuple) { return (py::cast(self).template cast<np_array_cf<Item>>())[tuple].template cast<py::array_t<Item>>(); }, py::keep_alive<0, 1>());
}
...
void define_get_item_1d_array(PyClass &pyClass)
{
    pyClass.def("__getitem__", [](const Class &self, int i) -> Item {
        ...
        {
            std::stringstream ss;
            ss << "Invalid indexing into a 1D array: got indices " << shape_to_string({i}) << " but array has dimensions " << shape_to_string({elems});
            throw std::runtime_error(ss.str());
        }
        if (i < 0)
        {
    ...
    pyClass.def(
        "__getitem__", [](const Class &self, py::slice slice) -> py::array_t<Item> { return (py::cast(self).template cast<np_array_cf<Item>>())[slice].template cast<py::array_t<Item>>(); },
        py::keep_alive<0, 1>());
}
...
void bindings_vpArray2D(py::class_<vpArray2D<T>, std::shared_ptr<vpArray2D<T>>> &pyArray2D)
{
    ...
    pyArray2D.def("numpy", [](vpArray2D<T> &self) -> np_array_cf<T> { return py::cast(self).template cast<np_array_cf<T>>(); }, numpy_fn_doc_writable, py::keep_alive<0, 1>());
    ...
    pyArray2D.def(py::init([](np_array_cf<T> &np_array) {
                  ...
                  R"doc(
                      Construct a 2D ViSP array by *
                      *copying * *a 2D numpy array.
                  : param np_array
...
void bindings_vpMatrix(py::class_<vpMatrix, std::shared_ptr<vpMatrix>, vpArray2D<double>> &pyMatrix)
{
    ...
    pyMatrix.def("numpy", [](vpMatrix &self) -> np_array_cf<double> { return py::cast(self).cast<np_array_cf<double>>(); }, numpy_fn_doc_writable, py::keep_alive<0, 1>());
    ...
    pyMatrix.def(py::init([](np_array_cf<double> np_array) {
                 ...
                 R"doc(
                     Construct a matrix by *
                     *copying * *a 2D numpy array.
                 : param np_array
                 : The numpy array to copy.)doc",
                 py::arg(" np_array "));
...
void bindings_vpRotationMatrix(py::class_<vpRotationMatrix, std::shared_ptr<vpRotationMatrix>, vpArray2D<double>> &pyRotationMatrix)
{
    ...
    pyRotationMatrix.def("numpy", [](vpRotationMatrix &self) -> np_array_cf<double> { return py::cast(self).cast<np_array_cf<double>>(); }, numpy_fn_doc_nonwritable, py::keep_alive<0, 1>());
    pyRotationMatrix.def(py::init([](np_array_cf<double> np_array) {
                         ...
                         R"doc(
                             Construct a rotation matrix by *
                             *copying * *a 2D numpy array.This numpy array should be of dimensions:math
                         :`3 \times 3` and be a valid rotation matrix.If it is not a rotation matrix, an exception will be raised.
                         : param np_array
...
void bindings_vpHomogeneousMatrix(py::class_<vpHomogeneousMatrix, std::shared_ptr<vpHomogeneousMatrix>, vpArray2D<double>> &pyHomogeneousMatrix)
{
    ...
    pyHomogeneousMatrix.def("numpy", [](vpHomogeneousMatrix &self) -> np_array_cf<double> { return py::cast(self).cast<np_array_cf<double>>(); }, numpy_fn_doc_nonwritable, py::keep_alive<0, 1>());
    ...
    pyHomogeneousMatrix.def(py::init([](np_array_cf<double> np_array) {
                            ...
                            R"doc(
                                Construct a homogeneous matrix by *
                                *copying * *a 2D numpy array.This numpy array should be of dimensions:math
                            :`4 \times 4` and be a valid homogeneous matrix.If it is not a homogeneous matrix, an exception will be raised.
...
void bindings_vpTranslationVector(py::class_<vpTranslationVector, std::shared_ptr<vpTranslationVector>, vpArray2D<double>> &pyTranslationVector)
{
    ...
    pyTranslationVector.def("numpy", [](vpTranslationVector &self) -> np_array_cf<double> { return py::cast(self).cast<np_array_cf<double>>(); }, numpy_fn_doc_writable, py::keep_alive<0, 1>());
    ...
    pyTranslationVector.def(py::init([](np_array_cf<double> np_array) {
                            ...
                            R"doc(
                                Construct a Translation vector by *
                                *copying * *a 1D numpy array of size 3.
...
void bindings_vpColVector(py::class_<vpColVector, std::shared_ptr<vpColVector>, vpArray2D<double>> &pyColVector)
{
    ...
    pyColVector.def("numpy", [](vpColVector &self) -> np_array_cf<double> { return py::cast(self).cast<np_array_cf<double>>(); }, numpy_fn_doc_writable, py::keep_alive<0, 1>());
    ...
    pyColVector.def(py::init([](np_array_cf<double> np_array) {
                    ...
                    R"doc(
                        Construct a column vector by *
                        *copying * *a 1D numpy array.
...
void bindings_vpRowVector(py::class_<vpRowVector, std::shared_ptr<vpRowVector>, vpArray2D<double>> &pyRowVector)
{
    ...
    pyRowVector.def(
        "numpy", [](vpRowVector &self) -> np_array_cf<double> { return np_array_cf<double>(get_buffer_info<vpRowVector>(self), py::cast(self)); }, numpy_fn_doc_writable, py::keep_alive<0, 1>());
