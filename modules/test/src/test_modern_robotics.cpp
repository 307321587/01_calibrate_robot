...
void realTimeControl(Eigen::MatrixXd B_matrix, Eigen::MatrixXd S_matrix, Eigen::MatrixXd M_matrix)
{
    ...
    Eigen::MatrixXd init_joint_positions_matrix(6, 1);
    {
        std::lock_guard<mutex> lock(mtx);
        for (int i = 0; i < 6; i++)
        {
            init_joint_positions_matrix(i) = joint_positions_global[i];
        }
    }
    int acc_sum_count = 20;
    Eigen::VectorXd target_vel(6);
    target_vel << 0, 0, 0, 0.001, 0, 0;
    Eigen::VectorXd target_acc(6);
    target_acc = target_vel / acc_sum_count;
    Eigen::VectorXd move_vector(6);
    move_vector << 0, 0, 0, 0, 0, 0;
    int acc_count = 0;
    // 输入不为q
    while (key != 'q' || acc_count >= 0)
    {
        if (key != 'q' && acc_count < acc_sum_count)
        {
            move_vector += target_acc;
            acc_count += 1;
        }
        if (key == 'q' && acc_count >= 0)
        {
            move_vector -= target_acc;
            acc_count -= 1;
        }
        ...
        // 矩阵乘以向量
        Eigen::VectorXd joint_velocities = inverse_jacobi_b * move_vector;
        ...
        // 输出计算结果
        // std::cout << "Joint velocities: " << joint_positions_matrix.transpose() << std::endl;
