...
int main()
{
    ...
    while (cnt < traj_sz)
    {
        ...
        cnt++;
        {
            std::lock_guard<mutex> lock(mtx);
            if (cnt < traj_sz - 1)
                ret = robotService.servoj(joint_angle, 0.005, 1, 1, false);
            else
                ret = robotService.servoj(joint_angle, 0.005, 1, 1, true);
        }

        if (ret != aubo_robot_namespace::InterfaceCallSuccCode)
        {
            robotService.robotServiceLeaveTcp2CanbusMode();
            robotService.stopServoj();
            return ret;
        }
